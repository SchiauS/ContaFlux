...
class OpenAIService
{
    public function chat(string $message, ?string $sessionId = null, array $context = []): array
    {
        $payload = [
            ...
            'messages' => [
                ['role' => 'system', 'content' => 'Assistant financiar pentru platforma ContaFlux.'],
    ...
    {
        $prompt = "Genereaza un rezumat financiar pentru perioada {$period}, cu indicatorii: " . implode(', ', $metrics ?: ['venituri', 'cheltuieli', 'marja']).". Returneaza bullet points concise.";
    ...
    private function post(string $path, array $payload): array
    {
        $response = Http::withToken(config('openai.key'))
            ->acceptJson()
            ->post(config('openai.base_url') . $path, $payload)
            ->throw()
            ->json();

        Log::info('OpenAI request', ['path' => $path, 'tokens' => data_get($response, 'usage.total_tokens')]);
